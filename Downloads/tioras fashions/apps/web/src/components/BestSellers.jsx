...
import { Star, ShoppingCart, Heart, ArrowRight, TrendingUp } from 'lucide-react';
...
const BestSellers = ({ headless = false }) => {
  ...
  useEffect(() => {
    const fetchBestSellers = async () => {
      try {
        ...
        // Simulate sales count and sort by it
        const productsWithSales = (response.products || []).map(p => ({
          ...p,
          salesCount: Math.floor(Math.random() * 1000) + 100 // Simulate sales count
      ...
      <section className="py-16 bg-muted/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-muted-foreground text-lg">Our most popular products</p>
          </div>
          {SkeletonGrid}
        </div>
      </section>
  ...
  if (products.length === 0) {
    const EmptyState = (
      <div className="text-center py-8">
        <p className="text-muted-foreground">No best sellers at the moment.</p>
      </div>
    );
        ...
        return (
          ...
          >
            <Link to={`/product/${product.id}`} className="relative aspect-[4/5] bg-muted overflow-hidden block">
              ...
              <Badge className="absolute top-3 left-3 bg-primary text-primary-foreground border-none shadow-md font-bold flex items-center gap-1">
            ...
            <div className="p-5 flex flex-col flex-1">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-bold text-primary">{product.salesCount}+ sold</span>
              </div>
